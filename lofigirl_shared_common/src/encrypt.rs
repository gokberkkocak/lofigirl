...
use aes_gcm::{
    ...
    aes::Aes256,
    Aes256Gcm, AesGcm, Key, KeyInit as _, Nonce,
};
...
static AES_CIPHER: LazyLock<AesGcm<Aes256, U12>> =
    LazyLock::new(|| Aes256Gcm::new(&ENCRYPTION_KEY));
static AES_IV: LazyLock<&'static GenericArray<u8, U12>> =
    LazyLock::new(|| Nonce::from_slice(ENCRYPTION_IV_BYTES));
...
impl Serialize for SecureString {
    ...
    where
        S: Serializer,
    {
        serializer.collect_str(&self.0.encrypt().map_err(serde::ser::Error::custom)?)
    }
}
